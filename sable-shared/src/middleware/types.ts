// Middleware types and helpers shared by every Sable Node service. Sable
// services run on Express but the types here are framework-agnostic
// (structurally compatible with Express's Request / Response / NextFunction)
// so sable-shared doesn't carry an Express runtime dep.
//
// Services declare the type merge in their own type-roots:
//
//   declare global {
//     namespace Express {
...
export interface SableRequest {
  ...
  /**
   * Raw request body bytes — retained by the body-parser when a route needs
   * to verify a byte-exact HMAC signature (service-auth, Stripe webhooks).
   */
  rawBody?: Buffer;
}
...
// ---------------------------------------------------------------------------
//
// Express's Request / Response satisfy these structurally — services pass
// their typed Express objects through and TypeScript accepts them. Keeping
// our own interfaces (rather than `import type { Request } from 'express'`)
// is what lets sable-shared stay free of any Express coupling, even at the
// type level. The set of fields here is exactly what Sable middleware reads
// or writes — nothing more.

export interface HttpRequest extends SableRequest {
  method: string;
  path: string;
  headers: Record<string, string | string[] | undefined>;
  /** Express-style header accessor — `req.header('x-foo')` lowercases the key. */
  header(name: string): string | undefined;
  /**
   * Parsed cookies — populated by cookie-parser. Optional because not every
   * route chain wires cookie-parser in; auth middleware checks for presence.
   */
