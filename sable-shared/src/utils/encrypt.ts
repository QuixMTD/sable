// DEK (Data Encryption Key) handling. The actual encryption is performed
// inside Postgres via the `enc()` / `dec()` helper functions in
// gateway-schema.sql, which call `pgp_sym_encrypt` / `pgp_sym_decrypt`
// with the per-session DEK pulled from `current_setting('app.dek')`.
//
// This file handles the application side: load the DEK once at boot (from
 ...
 * Throws if APP_DEK is not configured (which is intentional — services
 * that touch encrypted columns must not start without one).
 */
